...
**Not blocked on another issue since #7 landed.** Three of the four edits that
rename the executable are Chromium-owned, so they need an allowlist entry on the
integration branch; they are not listed in `minimum-chromium-hooks.md` §B and
adding them is #9's own work. The *check* was never blocked and should land
first, red, rather than arriving with the fix.
...
**No longer blocked on #7 for the fix** — `//astro` compiles and links as of
`b949a68`, so a generated `astro_install_modes.h` is reachable from the build.
What remains is minting the per-channel GUIDs, which is blocked by nothing and
blocks the first Windows release.
...
**Blocked on [#11](https://github.com/OxyHQ/Astro/issues/11) only.** #7's half
is done: `//astro` compiles, links into the browser, and its extra-parts hook is
proven to execute at runtime, so baseline finding 1 — that no build from
committed source carries Astro code — no longer describes the current tree. What
is still missing is a registered `astro://` scheme, which is #11's. Until that
lands this check must report **not-captured**, naming #11. It must not report a
pass, and it must not report a failure either: nothing was measured.
...
`b949a68`.** `//astro` now compiles and links, and the repository root *is*
`//astro` (`BUILD.gn:1-6`), so the rows that were waiting on the module existing
are no longer waiting on anything.
...
| W1, W3, W4, W5 — the *fixes* | the Chromium-owned edits listed in `product-manifest.md` §"Windows: `astro.exe`" | not a wait on another issue; they need an allowlist entry on the integration branch |
| W6 PE-metadata half | **— (unblocked)** | yes — `branding_file_path` is a GN argument and `//astro` exists |
| W6 icon half | the `chrome/app/theme/…` hook, pre-declared as #9-owned in `minimum-chromium-hooks.md` §B | see the open question in `product-manifest.md` — it may need no Chromium file at all |
| W7 | [#11](https://github.com/OxyHQ/Astro/issues/11) | no; report not-captured. #7's half is done |
| L1–L7 checks | — | yes |
| L6 fix | a Chromium-owned constant (`chrome/common/channel_info_posix.cc:151`) | same shape as the W1/W3/W4/W5 fixes |
| M1–M3, A1–A3 | a macOS/Android build | no; report not-captured |
| UA1, UA2 | a build carrying the Astro overlay | #7 supplies the module; these stay not-captured until a full build exists |
