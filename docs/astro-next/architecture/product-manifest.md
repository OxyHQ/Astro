...
matches the manifest, run by `tools/tests/cases/scheme-names-match-manifest.sh`
in the build-safety suite. **It is not generated yet, and that is a scope
decision rather than a shortcut:** generation belongs to `tools/lib/product.py
--generate`, which does not exist, and it would have to read a manifest at
`//astro/app/product.json`, which is the move §"Format and location" schedules
for when #7 lands. Building the pipeline against a path that is about to change,
and a script somebody else is writing, buys nothing the check does not already
buy today.

The checker is written so it becomes that generator's `--check` unchanged. What
it proves, all of it enforced in both directions:
...
An empty parse exits 2 rather than 0: a moved file, a renamed declaration shape
or a broken regex must not be indistinguishable from agreement.
