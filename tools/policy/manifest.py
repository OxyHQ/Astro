...
WHAT IS CHECKED
  ...
  dependency edges  `blocks` (this decision gates that issue) and `inputs_to`
                    (that issue reads this entry) are different relations, so
                    no entry may name one issue in both; and an inputs_to
                    edge has to say what is read.
  webui hosts       The WebUI data-source registry is a flat map keyed on the
                    bare host, so two entries claiming one host name are one
                    page. Declared host names are unique, and a page declares
                    its host and its scheme together or not at all.
...
def check(features: dict, endpoints: dict, seed: dict) -> tuple[list[str], dict]:
    ...
    # --- dependency edges, both manifests --------------------------------
    # `blocks` and `inputs_to` are different relations and were conflated
    # once already: ten decided entries declared blocks: [11], which read as
    # ten pending decisions #11 was waiting on. Naming an issue in both fields
    # would put the ambiguity straight back.
    for label, items in (("feature", feature_list), ("endpoint", endpoint_list)):
        for entry in items:
            where = f"{label} '{entry['id']}'"
            inputs = entry.get("inputs_to")
            if not inputs:
                continue
            count("inputs-edges")
            if not entry.get("inputs_reason"):
                errors.append(
                    f"{where}: inputs_to {inputs} without inputs_reason. The edge has to say what "
                    "the named issue reads, or it is indistinguishable from a blocker nobody "
                    "explained."
                )
            both = sorted(set(inputs) & set(entry.get("blocks") or []))
            if both:
                errors.append(
                    f"{where}: names {both} in both `blocks` and `inputs_to`. One relation or the "
                    "other -- an issue either waits on this decision or reads it."
                )

    # --- WebUI host namespace --------------------------------------------
    # url_data_manager_backend.cc:122 keys data sources on the bare host across
    # every WebUI scheme but chrome-untrusted, and url_data_source.cc:52-54
    # makes a later same-named registration win silently. Disjointness from
    # Chromium's own hosts needs a checkout and is asserted by #11's test; this
    # is the half that can be checked here.
    claimed_hosts: dict[str, list[str]] = {}
    for entry in feature_list:
        where = f"feature '{entry['id']}'"
        host = entry.get("webui_host")
        trust = entry.get("scheme_trust")
        if (host is None) != (trust is None):
            errors.append(
                f"{where}: declares only one of webui_host/scheme_trust. A page's name and the "
                "scheme it is served under are one row; half a row cannot be read by #11."
            )
        if host is None:
            continue
        count("webui-hosts")
        if host != "not-determined":
            claimed_hosts.setdefault(host, []).append(entry["id"])
    for host, owners in sorted(claimed_hosts.items()):
        if len(owners) > 1:
            errors.append(
                f"webui_host '{host}' is claimed by {owners}. The data-source registry is keyed on "
                "the bare host, so these are not two pages -- the later registration replaces the "
                "earlier one and the loser returns ERR_INVALID_URL."
            )
    ...
    if any(entry.get("inputs_to") for entry in feature_list + endpoint_list):
        if not counted.get("inputs-edges"):
            errors.append("vacuity: inputs_to edges exist but the rule examined none")
    if any("webui_host" in entry for entry in feature_list):
        if not counted.get("webui-hosts"):
            errors.append("vacuity: webui_host entries exist but the rule examined none")
...
def render_features(features: dict) -> str:
    ...
    pages = [entry for entry in entries if "webui_host" in entry]
    if pages:
        lines.append("## Internal page inventory")
        lines.append("")
        lines.append(
            "Which of the two internal schemes each page lands on, and the bare host its data "
            "source registers under. `scheme_trust` is not a preference: a document holding WebUI "
            "bindings has its network factory revoked, so a page whose own JavaScript must reach "
            "the network cannot be bound and therefore cannot be trusted. The rules that produce "
            "these two columns are `webui-scheme-trust-split` and `webui-host-namespace`."
        )
        lines.append("")
        lines.append("| Feature | Scheme | Host | Endpoints the page reaches | Owner |")
        lines.append("|---|---|---|---|---|")
        for entry in sorted(pages, key=lambda item: (item["scheme_trust"], item["id"])):
            network = ", ".join(f"`{name}`" for name in entry["network"])
            lines.append(
                "| `%s` | **%s** | `%s` | %s | #%s |"
                % (
    ...
    inputs = [entry for entry in entries if entry.get("inputs_to")]
    if inputs:
        lines.append("## Reference data another issue reads")
        lines.append("")
        lines.append(
            "A weaker relation than the table above, and the distinction is the point: these "
            "entries carry decided states, so the named issue cites them rather than waiting on "
            "them. Encoding them as blockers is what made #10 look like a 91-entry prerequisite "
            "for #11."
        )
        lines.append("")
        lines.append("| Feature | Read by | What is read |")
        lines.append("|---|---|---|")
        for entry in sorted(inputs, key=lambda item: (min(item["inputs_to"]), item["id"])):
            reads = ", ".join(f"#{number}" for number in entry["inputs_to"])
            lines.append(
                "| `%s` | %s | %s |"
                % (entry["id"], reads, _escape(entry.get("inputs_reason", "")))
            )
        lines.append("")
