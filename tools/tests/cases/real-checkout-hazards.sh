#!/usr/bin/env bash
# Three defects that only a real Chromium checkout exposed, and that every
...
#     writes `Cargo.toml.orig` beside every vendored Rust crate, so the real
...
# Executable lines only. These scripts describe the hazard in prose — the fix in
# sync-sources.sh is commented with the very construct it removed — and a raw
# grep would flag its own documentation.
...
# --------------------------------------------------------------------------
# Hazard 3 — a failed git fetch keeps nothing
# --------------------------------------------------------------------------
